...
<html>
	...
	$db = new MySQLDatabase("localhost", "GestionStock", "root", "");
		$row[] = $_GET["num"];
		$row[] = $_GET["date"];
		$row[] = $_GET["fournisseur"];
		$row[] = $_GET["produit"];
		$row[] = $_GET["qs"];
		$a = $db->updateR($_GET["produit"], intval($_GET["qs"]));
		$s = $db->insert($row, "reception");
		if($a){
	?>
	<h1 align="center">Reception bien enregistrer</h1>
	...
	?>
	<h1 align="center">Reception non enregistrer</h1>
	<form action="index.html">
